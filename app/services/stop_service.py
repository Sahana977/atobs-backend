"""Business rules for stops: validation, reference checks, uniqueness."""
import csv
import io
import sqlite3
...
from app.models.stop import Stop, FIELDS, ZONE_CHOICES
from app.repositories import stop_repo as repo

REQUIRED_FIELDS = ("stop_code", "name", "latitude", "longitude",)
MAX_PAGE_SIZE = 500
...
def _validate(data: dict, partial: bool = False) -> dict:
    ...
    # stop_code
    value = data.get("stop_code")
    if value is not None and "stop_code" not in errors:
        if not isinstance(value, str):
            errors["stop_code"] = "must be text"
        elif len(value) > 20:
            errors["stop_code"] = "must be at most 20 characters"
        else:
            data["stop_code"] = value.strip()
    ...
    # zone
    value = data.get("zone")
    if value is not None and "zone" not in errors:
        if not isinstance(value, str):
            errors["zone"] = "must be text"
        elif len(value) > 200:
            errors["zone"] = "must be at most 200 characters"
        elif value not in ZONE_CHOICES:
            errors["zone"] = "must be one of: " + ", ".join(ZONE_CHOICES)
        else:
            data["zone"] = value.strip()

    # has_shelter
    value = data.get("has_shelter")
    if value is not None and "has_shelter" not in errors:
        if not isinstance(value, bool):
            errors["has_shelter"] = "must be true or false"

    # is_accessible
    value = data.get("is_accessible")
    if value is not None and "is_accessible" not in errors:
        if not isinstance(value, bool):
            errors["is_accessible"] = "must be true or false"
...
def _check_unique(conn: sqlite3.Connection, data: dict, current_id: Optional[int] = None) -> None:
    ...
    if value is not None:
        existing = repo.get_by_stop_code(conn, value)
        if existing is not None and existing.id != current_id:
            raise ConflictError(f"stop with stop_code '{value}' already exists")


def create(data: dict) -> Stop:
...
def get(item_id: int) -> Stop:
    with database.session() as conn:
        item = repo.get(conn, item_id)
    if item is None:
        raise NotFoundError(f"stop {item_id} not found")
    return item
...
def update(item_id: int, changes: dict) -> Stop:
...
def delete(item_id: int) -> None:
    with database.session() as conn:
        ...
        try:
            ...
            raise ConflictError("stop is still referenced by other records") from exc


def bulk_create(items: list[dict]) -> list[Stop]:
