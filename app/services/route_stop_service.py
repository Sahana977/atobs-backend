"""Business rules for route stops: validation, reference checks, uniqueness."""
import csv
import io
import sqlite3
...
from app.models.route_stop import RouteStop, FIELDS
from app.repositories import route_stop_repo as repo
from app.repositories import route_repo
from app.repositories import stop_repo

REQUIRED_FIELDS = ("route_id", "stop_id", "sequence",)
MAX_PAGE_SIZE = 500
...
def _validate(data: dict, partial: bool = False) -> dict:
    ...
    if value is not None and "sequence" not in errors:
        if isinstance(value, bool) or not isinstance(value, int):
            errors["sequence"] = "must be a whole number"
        elif value < 1:
            errors["sequence"] = "must be at least 1"
        elif value > 200:
            errors["sequence"] = "must be at most 200"

    # minutes_from_start
    value = data.get("minutes_from_start")
    if value is not None and "minutes_from_start" not in errors:
        if isinstance(value, bool) or not isinstance(value, (int, float)):
            errors["minutes_from_start"] = "must be a number"
        elif value < 0:
            errors["minutes_from_start"] = "must be at least 0"
        elif value > 600:
            errors["minutes_from_start"] = "must be at most 600"
        else:
            data["minutes_from_start"] = float(value)
...
def create(data: dict) -> RouteStop:
...
def get(item_id: int) -> RouteStop:
    with database.session() as conn:
        item = repo.get(conn, item_id)
    if item is None:
        raise NotFoundError(f"route stop {item_id} not found")
    return item
...
def update(item_id: int, changes: dict) -> RouteStop:
...
def delete(item_id: int) -> None:
    with database.session() as conn:
        ...
        try:
            ...
            raise ConflictError("route stop is still referenced by other records") from exc


def bulk_create(items: list[dict]) -> list[RouteStop]:
